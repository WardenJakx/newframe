...
const isEnsName = (input: string) => input.toLowerCase().includes('.eth')

class AddAddress extends React.Component<any, any> {
  declare store: Store
  forms: any[]
  cancelEnsResolution: () => void
  ...
  async resolveEnsName(name: any) {
    return new Promise((resolve, reject) => {
      this.cancelEnsResolution = () => reject({ canceled: true })

      link.rpc('resolveEnsName', name, (err: any, resolvedAddress: any) => {
  ...
  createFromAddress(address: any) {
    link.rpc('createFromAddress', address, 'Watch Account', (err: any) => {
      if (err) {
        this.setError(err)
      } else {
  ...
  override render() {
    ...
    const itemClass = 'addAccountItem addAccountItemSmart addAccountItemAdding'
