...
function parseDeviceName(name: any) {
  return name.replace(/\s+/g, '-').substring(0, 14)
}

class AddHardwareLattice extends React.Component<any, any> {
  declare store: Store
  forms: any[]
  ...
  createLattice() {
    link.rpc('createLattice', this.state.deviceId, this.state.deviceName, (err: any, signer: any) => {
      if (err) {
