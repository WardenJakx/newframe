...
const navForward = (accountData: any) =>
...
const LocateKeystore = ({ addKeystore, error, setError }: any) => {
  useEffect(() => {
    if (!error) return
    setTimeout(() => {
      setError('')
...
const EnterKeystorePassword = ({ keystore }: any) => {
  const next = (keystorePassword: any) => {
    navForward({
      secret: keystore,
...
const LoadKeystore = ({ accountData }: any) => {
  ...
  const addKeystore = () => {
    ...
    setTimeout(() => {
      link.rpc('locateKeystore', (err: any, locatedKeystore: any) => {
...
const AddKeystore = ({ accountData }: any) => (
  <AddHotAccount
    title='Key Store'
