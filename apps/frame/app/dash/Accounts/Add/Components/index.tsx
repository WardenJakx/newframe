...
const navForward = async (newAccountType: any, accountData: any) =>
...
const removeLineBreaks = (str: any) => str.replace(/(\r\n|\n|\r)/gm, '')

const AddHotAccountWrapper = ({ children, title, svgName, summary, index }: any) => {
  return (
...
const EnterSecret = ({ newAccountType, validateSecret, title, autofocus }: any) => {
  const EMPTY_STATE = `Enter ${title}`
  const inputRef = useFocusableRef(autofocus, 100) as any
...
export function AddHotAccount({
  ...
  const onConfirm = () =>
    link.rpc(createSignerMethod, secret, password, ...creationArgs, (err: any, signer: any) => {
      if (err) {
        return navForward(newAccountType, {
          error: err
        })
