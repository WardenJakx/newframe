...
function mapToPreset(chainId: any, key: any) {
  return { text: key, value: `ethereum:${chainId}:${key}` }
}

const ConnectionIndicator = ({ className, connection }: any) => {
...
function getActiveConnection(primary: any, secondary: any) {
...
class ChainModule extends React.Component<any, any> {
  declare store: Store
  ...
  constructor(props: any, context?: any) {
    ...
    const { id, type } = props
    const primaryCustom =
      this.store('main.networks', type, id, 'connection.primary.custom') || this.customMessage
    const secondaryCustom =
      this.store('main.networks', type, id, 'connection.secondary.custom') || this.customMessage
  ...
  renderConnection(id: any, { primary, secondary }: any, blockHeight: any) {
  ...
  status(type: any, id: any, layer: any) {
  ...
  renderConnectionStatus(type: any, id: any, layer: any) {
  ...
  override render() {
    ...
    const toPreset = (key: any) => mapToPreset(id, key)
    ...
    const networkPresets = (NETWORK_PRESETS.ethereum as any)[id] || {}
    ...
    const customFocusHandler = (inputName: any) => {
    ...
    const customBlurHandler = (inputName: any) => {
      const stateKey = `${inputName}Custom`
      const state = this.state[stateKey]
      if (state === '') {
    ...
    const customChangeHandler = (e: any, inputName: any) => {
      e.preventDefault()
      ...
      clearTimeout((this as any)[timeoutName])
