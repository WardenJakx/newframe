...
class Account extends React.Component<any, any> {
  ...
  select() {
    if (this.store('selected.current') === this.props.id) {
      link.rpc('unsetSigner', this.props.id, (err: any) => {
        if (err) return console.log(err)
      })
      if (this.props.signer && this.store('main.accountCloseLock'))
        link.rpc('lockSigner', this.props.signer, (err: any) => {
          if (err) return console.log(err)
        })
    } else {
      ...
      link.rpc('setSigner', this.props.id, (err: any) => {
        if (err) return console.log(err)
      })
    }
  }
  ...
  isHotSigner(lastSignerType: any) {
  ...
  renderMenu() {
    ...
    if (((this.store('selected.current') === this.props.id) as any) & this.store('selected.open'))
  ...
  setHighlight(mode: any, index: any) {
  ...
  setSignerIndex(index: any) {
    this.locked = true
    link.rpc('setSignerIndex', index, (err: any) => {
  ...
  renderDetails() {
    ...
    requests = Object.keys(requests).filter((r: any) => requests[r].mode === 'normal')
