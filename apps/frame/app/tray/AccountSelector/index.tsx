...
class AccountSelector extends React.Component<any, any> {
  declare store: Store
  ...
  constructor(props: any, context?: any) {
    super(props, context)
    this.state = {
      accountFilter: this.store('panel.accountFilter') || ''
    }
  }
  ...
  renderAccountList() {
    const accounts = this.store('main.accounts')
    const sortedAccounts = (Object.values(accounts) as any[]).sort(byCreation)
    const filter = this.store('panel.accountFilter')

    const displayAccounts = sortedAccounts.filter(({ address, name, ensName, lastSignerType }: any) => {
      ...
      >
        ...
        <div className='accountSelectorScrollWrap'>
          {displayAccounts.length ? (
            displayAccounts.map((account: any, i: number) => (
              <AccountController
                key={account.id}
                {...account}
                index={i}
