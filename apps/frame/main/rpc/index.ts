...
const rpc: Record<string, (...args: any[]) => any> = {
  ...
  async flowCommand(command) {
    ...
    await (dapps as any).add(command.input, {}, (err: any, res: any) => {
      if (err || res) console.log(err, res)
    })
    await (dapps as any).launch(command.input, (err: any, res: any) => {
      if (err || res) console.log(err, res)
    })
  },
  addDapp(domain, options, cb) {
    if (!(domain.endsWith('.eth') || domain.endsWith('.xyz')))
      domain += '.eth'
      // console.log('addDapp', domain, options, cb)
    ;(dapps as any).add(domain, options, cb)
  },
  removeDapp(domain, cb) {
    ;(dapps as any).remove(domain, cb)
  },
  moveDapp(fromArea, fromIndex, toArea, toIndex, cb) {
    ;(dapps as any).move(fromArea, fromIndex, toArea, toIndex, cb)
  },
  launchDapp(domain, cb) {
    ;(dapps as any).launch(domain, cb)
  },
