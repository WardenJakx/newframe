...
export default class LedgerEthereumApp {
  ...
  async signTransaction(path: string, ledgerTx: TransactionData) {
    const signedTx = await sign(ledgerTx, (tx) => {
      // legacy transactions return their unsigned message pieces unencoded and need RLP encoding,
      // typed transactions return the serialized message directly
      const message = tx.getMessageToSign()
      const rawTxHex = Array.isArray(message)
        ? Buffer.from(encode(message)).toString('hex')
