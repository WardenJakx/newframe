...
const actions = {
  ...
  addSampleGasCosts: (u: U, netType: string, netId: number, samples: any) => {
  ...
  // Flow
  addDapp: (u: U, namehash: string, data: any, options = { docked: false, added: false }) => {
  ...
  setDappOpen: (u: U, ens: string, open: boolean) => {
    u('main.openDapps', (dapps: string[]) => {
      if (open) {
  ...
  removeDapp: (u: U, namehash: string) => {
  ...
  moveDapp: (u: U, fromArea: string, fromIndex: number, toArea: string, toIndex: number) => {
    u('main.dapp.map', (map: any) => {
  ...
  // Inventory
  setInventory: (u: U, address: string, inventory: any) => {
  ...
  mutePylonMigrationNotice: (u: U) => {
    u('main.mute.migrateToPylon', () => true)
  },
  migrateToPylonConnections: (u: U) => {
    ...
    u('main.networks.ethereum', (chains: any) => {
      Object.entries(chains as Record<string, any>).forEach(([id, chain]) => {
  ...
  updateDapp: (u: U, dappId: string, update: any) => {
    u('main.dapps', (dapps: any) => {
  ...
  addFrameView: (u: U, frameId: string, view: any) => {
    if (frameId && view) {
  ...
  setCurrentFrameView: (u: U, frameId: string, viewId: string) => {
    if (frameId) {
      u('main.frames', frameId, (frame: any) => {
  ...
  updateFrameView: (u: U, frameId: string, viewId: string, update: any) => {
  ...
  removeFrameView: (u: U, frameId: string, viewId: string) => {
    u('main.frames', frameId, 'views', (views: any) => {
      delete views[viewId]
      return views
    })
