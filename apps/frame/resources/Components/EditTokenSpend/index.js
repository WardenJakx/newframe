...
const isMax = (value) => toBigInt(value) === max
...
const EditTokenSpend = ({
  ...
  const toDecimal = (baseAmount) => formatUnits(toBigInt(baseAmount) ?? 0n, decimals)
  const fromDecimal = (decimalAmount) => (parseUnits(decimalAmount, decimals) ?? 0n).toString()
