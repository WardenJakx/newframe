...
async function waitForTask(taskName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const ps = spawn('bun', ['run', taskName], { stdio: 'inherit', shell: isWindows })
...
function launchFrame({ shutdown }: { shutdown: () => void }) {
  const bunProcess = spawn('bun', ['run', 'launch:dev'], { stdio: 'inherit', shell: isWindows })

  bunProcess.once('exit', () => {
