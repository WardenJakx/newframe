...
it('shows an error screen when ENS name resolution fails', async () => {
  jest.mocked(link.rpc).mockImplementationOnce((action: any, name: any, cb: any) => {
...
function setupComponent(_component?: any) {
  const { user } = render(<AddAddress />)

  return {
    user,
    enterText: async (text: any) => user.type(screen.getByLabelText('input address or ENS name'), text),
