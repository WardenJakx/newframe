...
describe('entering private key', () => {
  const setupComponent = () => {
    ...
    return {
      ...
      enterPrivateKey: async (text: any) => user.type(screen.getAllByRole('textbox')[0], text)
    }
  }
...
describe('confirming password', () => {
  const setupComponent = () => {
    ...
    return {
      ...
      enterPasswordConfirmation: async (text: any) => user.type(screen.getAllByRole('textbox')[2], text)
    }
  }
  ...
  it('should remove the previous screens related to adding an account from the navigation', async () => {
    jest.mocked(link.rpc).mockImplementationOnce((action: any, secret: any, passwd: any, cb: any) => {
