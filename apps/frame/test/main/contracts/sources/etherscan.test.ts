...
function mockApiResponse(
  ...
  const scope = nock(`https://${domain}`).get(path)

  if (timeout > 0) {
    // simulate a request that hangs until the client times out, without
    // scheduling a delayed reply that would fire after the request is aborted
    scope.reply(status, () => new Promise(() => {}), headers)
  } else {
    scope.reply(status, body, headers)
  }
}
...
describe('#fetchEtherscanContract', () => {
  ...
  const getPath = (apiKey: any) =>
    `/api?module=contract&action=getsourcecode&address=${contractAddress}&apikey=${apiKey}`

  const mockEtherscanApi = (status: any, response?: any, timeout?: any) => {
