...
// @ts-expect-error - multiformats ships no type declarations resolvable under this config
...
describe('#dappPathExists', () => {
  ...
  it('determines that a dapp exists in the dapp cache', async () => {
    ...
    expect(fs.access).toHaveBeenCalledWith(
      (expect as any).toMatchPath('/home/user/.config/DappCache/0xmydapp')
    )
...
describe('#isDappVerified', () => {
  ...
  beforeEach(() => {
    ;(importer as any).mockImplementation(() => createMockDirectoryImporter([dappImageCid, dappRootCid]))
  })
  ...
  it('looks in the dapp cache directory to verify the existing dapp content', async () => {
    ...
    const contentSourceGlob = (importer as any).mock.calls[0][0].getArgs()
    expect(contentSourceGlob).toStrictEqual([
      (expect as any).toMatchPath('/home/user/.config/DappCache/0xdapp'),
      '**'
    ])
  })
})
...
function createMockDirectoryImporter(cids: any) {
  return {
