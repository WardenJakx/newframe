...
let tokenLoader: any, mockEthProvider: any, mockNebula: any
...
describe('loading tokens', () => {
  it('loads the default token list initially', () => {
    ...
    expect(tokens.find((token: any) => token.name === 'Aave')).toBeTruthy()
  })
...
describe('#getBlacklist', () => {
  ...
  it('returns all blacklisted tokens', () => {
    const blacklistedTokens = tokenLoader.getBlacklist().map((t: any) => t.name)

    expect(blacklistedTokens).toStrictEqual(['Optimism', 'Minereum'])
  })

  it('returns blacklisted tokens from a specific chain', () => {
    const blacklistedTokens = tokenLoader.getBlacklist([137]).map((t: any) => t.name)
