...
// @ts-expect-error - fs-extra ships no type declarations
...
describe('Ring signer', () => {
  ...
  test('Add private key', (done) => {
    try {
      ...
      signer.addPrivateKey(privateKey, PASSWORD, (err: any) => {
        expect(err).toBe(null)
  ...
  test('Remove private key', (done) => {
    try {
      ...
      signer.removePrivateKey(0, PASSWORD, (err: any) => {
        expect(err).toBe(null)
  ...
  test('Add private key from keystore', (done) => {
    try {
      ...
      signer.addKeystore(keystore, 'test', PASSWORD, (err: any) => {
        expect(err).toBe(null)
  ...
  test('Unlock', (done) => {
    try {
      signer.unlock(PASSWORD, (err: any) => {
        expect(err).toBe(null)
        done()
      })
    } catch (e) {
