...
async function connectEthApp() {
  ;(Eth as any).mock.instances[0].getAppConfiguration.mockResolvedValue({ version: '2.0.1' })
...
function verifyDone(done: any, expectations: any) {
  verify(done, expectations, true)
}

function verify(done: any, expectations: any, complete = false) {
  try {
    expectations()
    if (complete) done()
  } catch (e) {
...
function verifyPromise(resolve: any, reject: any, expectations: any) {
  try {
    expectations()
    resolve()
  } catch (e) {
...
beforeEach(async () => {
  ...
  ledger = new (Ledger as any)('usb-path')
  ledger.derivation = Derivation.legacy

  await ledger.open()
  ;(Eth as any).mock.instances[0].deriveAddresses.mockImplementation(() => Promise.resolve(addresses))
})
...
describe('#connect', () => {
  describe('when the eth app is open', () => {
    beforeEach(() => {
      ;(Eth as any).mock.instances[0].getAppConfiguration.mockResolvedValue({ version: '1.9.2' })
    })
    ...
    it('detects that the app is locked', (done) => {
      ...
      ;(Eth as any).mock.instances[0].getAddress.mockRejectedValue({ statusCode: 27404 })
  ...
  describe('when the eth app is not open', () => {
    ...
    statusCodes.forEach((code) => {
      it(`sets the status to wrong application and disconnects the signer when the status code is ${code}`, async () => {
        ;(Eth as any).mock.instances[0].getAppConfiguration.mockRejectedValue({ statusCode: code })
...
describe('#deriveAddress', () => {
  ...
  it('derives live addresses', (done) => {
    ;(Eth as any).mock.instances[0].getAddress.mockImplementation((path: any) => {
...
describe('#verifyAddress', () => {
  beforeEach(async () => {
    await connectEthApp()
    ;(Eth as any).mock.instances[0].getAddress.mockImplementation(
      (path: any) =>
        new Promise((resolve) => {
          resolve({
            address:
  ...
  const errorCases = [
    ...
    },
    {
      ...
      setup: () => (Eth as any).mock.instances[0].getAddress.mockRejectedValue({ statusCode: 27013 })
    },
    {
      testCase: 'there is a communication error',
      setup: () => (Eth as any).mock.instances[0].getAddress.mockRejectedValue({ statusCode: -1 })
    },
    {
...
signingMethods.forEach((signingMethod) => {
  ...
  describe(`#${signingMethod}`, () => {
    ...
    it(`signs a ${signType}`, (done) => {
      ;(Eth as any).mock.instances[0][signingMethod].mockImplementation(
        (path: any) =>
          new Promise((resolve) => {
            resolve(
              path === "44'/60'/0'/3"
      ...
      ledger[signingMethod](3, 'hello, Frame!', (err: any, signature: any) => {
        verifyDone(done, () => {
    ...
    it('fails if the signing request is rejected by the user', (done) => {
      ;(Eth as any).mock.instances[0][signingMethod].mockRejectedValue({ statusCode: 27013 })
    ...
    const errorCases = [
      {
        testCase: 'there is a communication error',
        setup: () => (Eth as any).mock.instances[0][signingMethod].mockRejectedValue({ statusCode: -1 })
      },
      {
        testCase: 'the eth app is not initialized',
        setup: () => (ledger.eth = undefined)
...
describe('#signTypedData', () => {
  ...
  it('signs v4 typed data', (done) => {
    ;(Eth as any).mock.instances[0].signTypedData.mockImplementation(
      (path: any) =>
        new Promise((resolve) => {
          resolve(
            path === "44'/60'/0'/5"
  ...
  it('fails if the signing request is rejected by the user', (done) => {
    ;(Eth as any).mock.instances[0].signTypedData.mockRejectedValue({ statusCode: 27013 })
  ...
  it('fails if the signing request is invalid', (done) => {
    ;(Eth as any).mock.instances[0].signTypedData.mockRejectedValue({
      statusCode: 99901,
  ...
  const errorCases = [
    {
      testCase: 'there is a communication error',
      setup: () => (Eth as any).mock.instances[0].signTypedData.mockRejectedValue({ statusCode: -1 })
    },
    {
      testCase: 'the eth app is not initialized',
      setup: () => (ledger.eth = undefined)
