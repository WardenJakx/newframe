import { displayValueData } from '../../../resources/utils/displayValue'

describe('wei', () => {
  it('should return a wei value', () => {
    const displayValue = displayValueData(356)
    expect(displayValue.wei()).toStrictEqual({ displayValue: '356', value: 356n })
  })

  it('should return a zero wei value', () => {
    const displayValue = displayValueData(0)
    expect(displayValue.wei()).toStrictEqual({ displayValue: '0', value: 0n })
  })
})

describe('gwei', () => {
  it('should return a gwei value', () => {
    const displayValue = displayValueData(356e9)
    expect(displayValue.gwei()).toStrictEqual({ displayValue: '356', value: 356 })
  })

  it('should not return a gwei value of more than 6dp', () => {
    const displayValue = displayValueData(356e-18)
    expect(displayValue.gwei()).toStrictEqual({ displayValue: '0', value: 0 })
  })

  it('should return a zero gwei value', () => {
    const displayValue = displayValueData(0)
    expect(displayValue.gwei()).toStrictEqual({ displayValue: '0', value: 0 })
  })
})

describe('fiat currency', () => {
  it('should return ? when no currency rate is provided', () => {
    const value = displayValueData(356e24)
    expect(value.fiat()).toStrictEqual({
      displayValue: '?',
      value: 0
    })
  })

  it('should return "testnet zero" when isTestnet is true', () => {
    const value = displayValueData(356e24, { currencyRate: { price: 1.3 }, isTestnet: true })
    expect(value.fiat()).toStrictEqual({
      displayValue: '?',
      value: 0
    })
  })

  describe('when displaying decimals', () => {
    it('should return a value of less than a cent', () => {
      const value = displayValueData(356e12, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        approximationSymbol: '<',
        displayValue: '0.01',
        value: 0.000356
      })
    })

    it('should return a value less than 1000 with a fixed 2dp', () => {
      const value = displayValueData(999999e15, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayValue: '999.99',
        value: 999.999
      })
    })

    it('should return a zero value', () => {
      const value = displayValueData(0, { currencyRate: { price: 1.3 } })
      expect(value.fiat()).toStrictEqual({
        displayValue: '0.00',
        value: 0
      })
    })
  })

  describe('when not displaying decimals', () => {
    it('should return a value of less than a dollar', () => {
      const value = displayValueData(356e12, { currencyRate: { price: 1 } })
      expect(value.fiat({ displayDecimals: false })).toStrictEqual({
        approximationSymbol: '<',
        displayValue: '1',
        value: 0.000356
      })
    })

    it('should return a value less than 1000 without decimals', () => {
      const value = displayValueData(999999e15, { currencyRate: { price: 1 } })
      expect(value.fiat({ displayDecimals: false })).toStrictEqual({
        displayValue: '999',
        value: 999.999
      })
    })

    it('should return a zero value', () => {
      const value = displayValueData(0, { currencyRate: { price: 1.3 } })
      expect(value.fiat({ displayDecimals: false })).toStrictEqual({
        displayValue: '0',
        value: 0
      })
    })
  })

  describe('shorthand large values', () => {
    it('should return a value of millions to 2dp', () => {
      const value = displayValueData(356253e20, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'million',
          shortName: 'M'
        },
        displayValue: '35.62',
        value: 35625300
      })
    })

    it('should round down a value of millions to 2dp', () => {
      const value = displayValueData(356259e20, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'million',
          shortName: 'M'
        },
        displayValue: '35.62',
        value: 35625900
      })
    })

    it('should return an exact value of millions', () => {
      const value = displayValueData(35e24, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'million',
          shortName: 'M'
        },
        displayValue: '35',
        value: 35000000
      })
    })

    it('should return a value of billions to 2dp', () => {
      const value = displayValueData(35.6253e27, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'billion',
          shortName: 'B'
        },
        displayValue: '35.62',
        value: 35625300000
      })
    })

    it('should round down a value of billions to 2dp', () => {
      const value = displayValueData(35.6259e27, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'billion',
          shortName: 'B'
        },
        displayValue: '35.62',
        value: 35625900000
      })
    })

    it('should return an exact value of billions', () => {
      const value = displayValueData(35e27, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'billion',
          shortName: 'B'
        },
        displayValue: '35',
        value: 35000000000
      })
    })

    it('should return a value of trillions to 2dp', () => {
      const value = displayValueData(35.6253e30, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'trillion',
          shortName: 'T'
        },
        displayValue: '35.62',
        value: 35625300000000
      })
    })

    it('should round down a value of trillions to 2dp', () => {
      const value = displayValueData(35.6259e30, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'trillion',
          shortName: 'T'
        },
        displayValue: '35.62',
        value: 35625900000000
      })
    })

    it('should return an exact value of trillions', () => {
      const value = displayValueData(35e30, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'trillion',
          shortName: 'T'
        },
        displayValue: '35',
        value: 35000000000000
      })
    })

    it('should return a value of quadrillions to 2dp', () => {
      const value = displayValueData(35.6253e33, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '35.62',
        value: 3.56253e16
      })
    })

    it('should round down a value of quadrillions to 2dp', () => {
      const value = displayValueData(35.6259e33, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '35.62',
        value: 3.56259e16
      })
    })

    it('should return an exact value of quadrillions', () => {
      const value = displayValueData(35e33, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '35',
        value: 3.5e16
      })
    })

    it('should return a maximum value', () => {
      const value = displayValueData(356e50, { currencyRate: { price: 1 } })
      expect(value.fiat()).toStrictEqual({
        approximationSymbol: '>',
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '999,999',
        value: 3.56e34
      })
    })
  })
})

describe('ether currency', () => {
  describe('when displaying decimals', () => {
    it('should return a value of less than 1000 gwei', () => {
      const value = displayValueData(356e8)
      expect(value.ether()).toStrictEqual({
        approximationSymbol: '<',
        displayValue: '0.000001',
        value: 3.56e-8
      })
    })

    it('should return a value less than 1000 with a fixed 3dp', () => {
      const value = displayValueData(998.5678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '998.567',
        value: 998.5678111111
      })
    })

    it('should return a value less than 100 with a fixed 4dp', () => {
      const value = displayValueData(99.85678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '99.8567',
        value: 99.85678111111
      })
    })

    it('should return a value less than 10 with a fixed 5dp', () => {
      const value = displayValueData(9.985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '9.98567',
        value: 9.985678111111
      })
    })

    it('should return a value less than 1 with a fixed 6dp', () => {
      const value = displayValueData(0.9985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '0.998567',
        value: 0.9985678111111
      })
    })

    it('should return a value less than 0.1 with a fixed 6dp', () => {
      const value = displayValueData(0.09985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '0.099856',
        value: 0.09985678111111
      })
    })

    it('should return a value less than 0.01 with a fixed 6dp', () => {
      const value = displayValueData(0.009985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '0.009985',
        value: 0.009985678111111
      })
    })

    it('should return a value less than 0.001 with a fixed 6dp', () => {
      const value = displayValueData(0.0009985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '0.000998',
        value: 0.0009985678111111
      })
    })

    it('should return a value less than 0.0001 with a fixed 6dp', () => {
      const value = displayValueData(0.00009985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '0.000099',
        value: 0.00009985678111111
      })
    })

    it('should return a value less than 0.00001 with a fixed 6dp', () => {
      const value = displayValueData(0.000009985678111111e18)
      expect(value.ether()).toStrictEqual({
        displayValue: '0.000009',
        value: 0.000009985678111111
      })
    })

    it('should return a zero value', () => {
      const value = displayValueData(0)
      expect(value.ether()).toStrictEqual({
        displayValue: '0',
        value: 0
      })
    })
  })

  describe('when not displaying decimals', () => {
    it('should return a value of less than 1', () => {
      const value = displayValueData(356e12)
      expect(value.ether({ displayDecimals: false })).toStrictEqual({
        approximationSymbol: '<',
        displayValue: '1',
        value: 0.000356
      })
    })

    it('should return a value less than 1000 without decimals', () => {
      const value = displayValueData(999999e15)
      expect(value.ether({ displayDecimals: false })).toStrictEqual({
        displayValue: '999',
        value: 999.999
      })
    })

    it('should return a zero value', () => {
      const value = displayValueData(0)
      expect(value.ether({ displayDecimals: false })).toStrictEqual({
        displayValue: '0',
        value: 0
      })
    })
  })

  describe('shorthand large values', () => {
    it('should return a value of millions to 2dp', () => {
      const value = displayValueData(35.6253e24)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'million',
          shortName: 'M'
        },
        displayValue: '35.62',
        value: 35625300
      })
    })

    it('should round down a value of millions to 2dp', () => {
      const value = displayValueData(35.6259e24)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'million',
          shortName: 'M'
        },
        displayValue: '35.62',
        value: 35625900
      })
    })

    it('should return an exact value of millions', () => {
      const value = displayValueData(35e24)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'million',
          shortName: 'M'
        },
        displayValue: '35',
        value: 35000000
      })
    })

    it('should return a value of billions to 2dp', () => {
      const value = displayValueData(35.6253e27)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'billion',
          shortName: 'B'
        },
        displayValue: '35.62',
        value: 35625300000
      })
    })

    it('should round down a value of billions to 2dp', () => {
      const value = displayValueData(35.6259e27)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'billion',
          shortName: 'B'
        },
        displayValue: '35.62',
        value: 35625900000
      })
    })

    it('should return an exact value of billions', () => {
      const value = displayValueData(35e27)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'billion',
          shortName: 'B'
        },
        displayValue: '35',
        value: 35000000000
      })
    })

    it('should return a value of trillions to 2dp', () => {
      const value = displayValueData(35.6253e30)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'trillion',
          shortName: 'T'
        },
        displayValue: '35.62',
        value: 35625300000000
      })
    })

    it('should round down a value of trillions to 2dp', () => {
      const value = displayValueData(35.6259e30)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'trillion',
          shortName: 'T'
        },
        displayValue: '35.62',
        value: 35625900000000
      })
    })

    it('should return an exact value of trillions', () => {
      const value = displayValueData(35e30)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'trillion',
          shortName: 'T'
        },
        displayValue: '35',
        value: 35000000000000
      })
    })

    it('should return a value of quadrillions to 2dp', () => {
      const value = displayValueData(35.6253e33)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '35.62',
        value: 3.56253e16
      })
    })

    it('should round down a value of quadrillions to 2dp', () => {
      const value = displayValueData(35.6259e33)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '35.62',
        value: 3.56259e16
      })
    })

    it('should return an exact value of quadrillions', () => {
      const value = displayValueData(35e33)
      expect(value.ether()).toStrictEqual({
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '35',
        value: 3.5e16
      })
    })

    it('should return a maximum value', () => {
      const value = displayValueData(356e50)
      expect(value.ether()).toStrictEqual({
        approximationSymbol: '>',
        displayUnit: {
          fullName: 'quadrillion',
          shortName: 'Q'
        },
        displayValue: '999,999',
        value: 3.56e34
      })
    })
  })
})
