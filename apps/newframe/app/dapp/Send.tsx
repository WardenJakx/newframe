...
class Send extends React.Component<SendProps, any> {
  declare store: Store
  selectBalanceSummaries = createBalanceSummarySelector()

  constructor(props: SendProps, context?: any) {
    super(props, context)
    this.state = {
      amount: '1',
      error: '',
      ...
      recipientInput: '',
      recipientOpen: true,
      selectedAssetKey: props.assetId || '',
      status: '',
      submitting: false,
      tokenOpen: false,
  ...
  getSelectedAsset() {
    ...
    const selectedKey = this.state.selectedAssetKey || ''
    const selectedAsset = resolveSendAssetFromRouteAssetId(selectedKey, balances)

    return selectedAsset ? createDisplayBalance(selectedAsset) : null
  }
  ...
  selectAsset(asset: any) {
    this.setState({
      error: '',
      selectedAssetKey: toCanonicalAssetId(asset),
      tokenOpen: false
    })
  }
  ...
  renderTokenSelector(asset: any) {
    ...
    const selectedBalance = balances.find(
      (balance: BalanceSummary) => toCanonicalAssetId(balance) === selectedKey
    )
    const menuBalances =
      selectedBalance &&
      !visibleBalances.some((balance: BalanceSummary) => toCanonicalAssetId(balance) === selectedKey)
