...
export interface SendEthereumNetwork {
  id?: number
  isTestnet?: boolean
  name?: string
  on?: boolean
...
export interface SendEthereumNetworkMeta {
  icon?: string
  nativeCurrency?: {
    decimals?: number
    icon?: string
...
export interface SendSelectorValue {
  accounts: SendAccount[]
  balanceSummaries: BalanceSummary[]
  currentAccount: SendAccount | null
  networks: Record<string | number, SendEthereumNetwork>
  networksMeta: Record<string | number, SendEthereumNetworkMeta>
}

interface SendRendererState extends RendererState {
  main?: {
    accountOrder?: string[]
    accounts?: Record<string, SendAccount>
    balances?: Record<string, Balance[]>
    networks?: {
      ethereum?: Record<string | number, SendEthereumNetwork>
    }
    networksMeta?: {
      ethereum?: Record<string | number, SendEthereumNetworkMeta>
    }
...
function createOrderedAccountsSelector() {
  let previousAccountsById: Record<string, SendAccount> | undefined
  let previousAccountOrder: string[] | undefined
  let previousOrderedAccounts: SendAccount[] = []

  return (accountsById: Record<string, SendAccount> = {}, accountOrder?: string[]) => {
...
export function createSendViewSelector() {
  const selectBalanceSummaries = createBalanceSummarySelector()
  const selectOrderedAccounts = createOrderedAccountsSelector()
  let previousResult: SendSelectorValue | null = null

  return (state: SendRendererState): SendSelectorValue => {
