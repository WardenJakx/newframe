...
import { formatDisplayDecimal, isUnlimited, toBigInt } from '../../../../../../resources/utils/numbers'
...
class TxSending extends React.Component<any, any> {
  ...
  copyAddress(data: any) {
    ...
    setTimeout((_?: any) => this.setState({ copied: false }), 1000)
  }
  override render() {
    ...
    if (actionClass === 'erc20') {
      if (actionType === 'transfer') {
        ...
        const hasTokenUSD = Boolean(rate?.usd && !isTestnet)
                ...
                {hasTokenUSD ? (
                  <ClusterValue>
                    ...
                    <DisplayValue
                      type='fiat'
                      value={amount}
                      valueDataParams={{ currencyRate: rate.usd, isTestnet, decimals }}
                      currencySymbol='$'
                    />
                  </ClusterValue>
                ) : null}
        ...
        const revoke = toBigInt(amount) === 0n
