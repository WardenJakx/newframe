...
class TxMain extends React.Component<any, any> {
  declare store: Store
  ...
  getReplacementStatus(req: any, r: any) {
    const replacementStatus = { replacement: false, possible: true, notice: '' }
    if (req.mode !== 'monitor' && req.data.nonce) {
      const requests = Object.keys(r || {}).map((key: any) => r[key])
      const monitor = requests.filter((req: any) => req.mode === 'monitor')
      const monitorFilter = monitor.filter((r: any) => r.status !== 'error')
      const existingNonces = monitorFilter.map((m: any) => m.data.nonce)
      existingNonces.forEach((nonce: any, i: number) => {
        if (req.data.nonce === nonce) {
          ...
          if (monitorFilter[i].status === 'confirming' || monitorFilter[i].status === 'confirmed') {
            ...
            req.data.gasPrice &&
            parseInt(monitorFilter[i].data.gasPrice, 'hex' as any) >=
              parseInt(req.data.gasPrice, 'hex' as any)
          ) {
            replacementStatus.possible = false
            ...
            Math.ceil(parseInt(monitorFilter[i].data.maxPriorityFeePerGas, 'hex' as any) * 1.1) >
              parseInt(req.data.maxPriorityFeePerGas, 'hex' as any) &&
            Math.ceil(parseInt(monitorFilter[i].data.maxFeePerGas, 'hex' as any) * 1.1) >
              parseInt(req.data.maxFeePerGas, 'hex' as any)
          ) {
            replacementStatus.possible = false
