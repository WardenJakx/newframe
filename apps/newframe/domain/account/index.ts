...
type NamedAccount = AccountIdentity & {
  created: string
  lastSignerType: string
  name: string
}

export function hasAddress(account: AccountIdentity, address = '') {
  return account.id.toLowerCase() === address.toLowerCase()
}

export const isDefaultAccountName = ({ name, lastSignerType }: NamedAccount) =>
  name.toLowerCase() === getDefaultAccountName(lastSignerType)

const getDefaultAccountName = (type: string) => `${getSignerDisplayType(type)} account`

export function accountSort(a: NamedAccount, b: NamedAccount) {
  try {
