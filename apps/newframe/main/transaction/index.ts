...
function hexToBigInt(value = '0x0') {
  return BigInt(addHexPrefix(value || '0'))
}

function calculateMaxFeePerGas(maxBaseFee: string, maxPriorityFee: string) {
  const maxFeePerGas = (hexToBigInt(maxPriorityFee) + hexToBigInt(maxBaseFee)).toString(16)
...
function populate(rawTx: TransactionData, chainConfig: Common, gas: Gas): TransactionData {
  ...
  if (!useFrameMaxFeePerGas && !useFrameMaxPriorityFeePerGas) {
    // return tx unaltered when we are using no Newframe-supplied values
    return txData
  }
  ...
  // if no valid dapp-supplied value for maxPriorityFeePerGas we use the Newframe-supplied value
  txData.maxPriorityFeePerGas = useFrameMaxPriorityFeePerGas
    ? addHexPrefix(hexToBigInt(maxPriorityFee).toString(16))
