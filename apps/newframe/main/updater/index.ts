...
class Updater {
  ...
  start() {
    ...
    const runCheck = () => {
      if (this.startCheck()) check()
    }

    this.setupCheck = setTimeout(() => {
      runCheck()
      this.pendingCheck = setInterval(runCheck, UPDATE_INTERVAL)
    }, this.nextCheckDelay())
  }
  ...
  private nextCheckDelay() {
    const now = Date.now()
    const lastChecked = this.lastCheckedAt()

    if (!lastChecked || lastChecked > now) return STARTUP_CHECK_DELAY

    const delay = UPDATE_INTERVAL - (now - lastChecked)

    return delay > 0 ? delay : STARTUP_CHECK_DELAY
  }
  ...
  private startCheck() {
    ...
    if (lastChecked && lastChecked <= now && now - lastChecked < UPDATE_INTERVAL) {
      log.verbose('Skipping update check, checked recently', {
        lastChecked,
        nextCheckAt: lastChecked + UPDATE_INTERVAL
      })
      return false
    }
