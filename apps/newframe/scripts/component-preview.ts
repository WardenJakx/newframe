...
async function main() {
  ...
  if (positionals.length !== 1 || !fixture) throw new Error(usage)
  for (const path of [fixture, values.check, values.screenshot, values.browser]) {
    if (path && !isAbsolute(path)) throw new Error(`Use an absolute path: ${path}`)
  }
  if (values.check && !values.screenshot) throw new Error('--check requires --screenshot')
  ...
  if (values.screenshot) {
    if (!values.screenshot.endsWith('.png')) throw new Error('--screenshot must end in .png')
    if (
  ...
  for (const path of [fixture, values.check]) {
    if (path && !(await Bun.file(path).exists())) throw new Error(`File not found: ${path}`)
  }
...
<style>html,body,#preview{height:100%;width:100%;margin:0}</style>
      ...
      plugins: [
        {
          ...
          setup(build) {
            build.onResolve({ filter: /^[^./]/ }, ({ path, importer }) => {
              ...
              if (importer === fixture || importer === entry) return { path: Bun.resolveSync(path, appDir) }
    ...
    if (!result.success) throw new Error(`Preview build failed:\n${result.logs.join('\n')}`)
    stopped.signal.throwIfAborted()
    ...
    if (!index) throw new Error('Preview build produced no HTML')
    outputs.set('/', index)
    server = Bun.serve({
      hostname: '127.0.0.1',
      port: 0,
    ...
    await context.route('**/*', async (route) => {
      ...
      if (new URL(requestUrl).origin === new URL(url).origin) return route.continue()
    ...
    const assertHealthy = () => {
      stopped.signal.throwIfAborted()
      if (errors.size) throw new Error('Browser reported errors')
    }
    ...
    if (errors.size) throw new Error(`Preview failed:\n${[...errors].join('\n')}`, { cause: error })
    throw error
  } finally {
