...
function cleanOptionalAmount(amount: unknown, label: string) {
  ...
  if (!positiveFlashNumber(clean)) {
    validationError(`Local Flash ${label} must be a positive decimal`)
  }
...
function localTriggers(value: unknown): LocalTrigger[] {
  ...
  if (!Array.isArray(value)) {
    validationError('Local Flash triggers must be an array')
  }
  ...
  return value.map((trigger, index) => {
    ...
    if (!notionalPrice) {
      validationError(`Local Flash triggers[${index}].notionalPrice is required`)
    }
...
function ensureNoTriggers(triggers: LocalTrigger[], orderType: FlashOrderType) {
  if (triggers.length) {
    validationError(`Local Flash triggers are not allowed for ${orderType}`)
  }
}
...
function validateOrderParameters(
  ...
  if (orderType === FLASH_MARKET_ORDER_TYPE) {
    ...
    if (limitNotionalPrice) {
      validationError('Local Flash limitNotionalPrice is not allowed for market')
    }
    if (expireTime) {
      validationError('Local Flash expireTime is not allowed for market')
    }
    if (startTime) {
      validationError('Local Flash startTime is not allowed for market')
    }
  }
  ...
  if (orderType === FLASH_LIMIT_ORDER_TYPE) {
    ...
    ensureNoTwapFields(body, orderType)
    if (!limitNotionalPrice) {
      validationError('Local Flash limitNotionalPrice is required for limit')
    }
    if (startTime) {
      validationError('Local Flash startTime is not allowed for limit')
    }
  }
  ...
  if (orderType === FLASH_TWAP_ORDER_TYPE) {
    ensureNoTriggers(triggers, orderType)
    if (expireTime) {
      validationError('Local Flash expireTime is not allowed for TWAP')
    }
  ...
  if (
    ...
    ensureNoTwapFields(body, orderType)
    if (startTime) {
      validationError(`Local Flash startTime is not allowed for ${orderType}`)
    }
...
function settlementInputToken(spentAsset: FlashAsset) {
  ...
  if (!asset.address) {
    throw new Error(`Cannot settle unsupported input asset ${asset.symbol}`)
  }
...
async function readAllowance(asset: FlashAsset, owner: string, spender: string) {
  if (!asset.address) {
    throw new Error(`Cannot check allowance for ${asset.symbol}`)
  }
...
async function buildApprovalAction({
  ...
  if (settlementAsset.isNative || !settlementAsset.address) {
    return null
  }
...
function orderTypedData({
  ...
  return {
    ...
    message: {
      ...
      orderType,
      qty,
      settlementAsset: isCrossChain
        ? LOCAL_MOCK_FLASH_SETTLEMENT_ADDRESS
        : settlementSpentAsset(spentAsset).symbol
    }
  }
}

function localBridgeQuoteId({
  body,
  contraAsset,
  targetAsset
}: {
...
async function buildQuote(body: Record<string, unknown>) {
  const accountAddress = checksumAddress(stringValue(body.funderAddress), 'funder')
  const recipient = stringValue(body.recipientAddress)
  const recipientAddress = recipient ? checksumAddress(recipient, 'recipient') : accountAddress
  if (recipientAddress !== accountAddress) {
    validationError('Local Flash recipientAddress must match funderAddress')
  }
  const targetAsset = localAssetFromAddress(body.targetChain, body.targetAsset, 'target')
  const contraAsset = localAssetFromAddress(body.contraChain, body.contraAsset, 'contra')
  ...
  if (!amount) {
    validationError('Local Flash quote requires a positive qty')
  }
  ...
  const wrapAction =
    !chains.isCrossChain && spentAsset.isNative
      ? buildWrapAction({ accountAddress, amount: qty, amountRaw: inputAmountRaw })
      : null
  const approvalAction = chains.isCrossChain
    ? null
    : await buildApprovalAction({
        accountAddress,
        amount: qty,
  ...
  const bridgeQuoteId = chains.isCrossChain ? localBridgeQuoteId({ body, contraAsset, targetAsset }) : ''
  const quoteId = chains.isCrossChain ? '' : `local-quote-${crypto.randomUUID()}`
  const typedData = orderTypedData({
    accountAddress,
    chainId: chains.spentChainId,
    isCrossChain: chains.isCrossChain,
    orderType,
    quoteId: quoteId || bridgeQuoteId,
    qty,
    side,
    spentAsset
  })
  ...
  const response = {
    ...
    orderType,
    side,
    targetChain: getFlashChainSlug(targetAsset.chainId),
    contraChain: getFlashChainSlug(contraAsset.chainId),
    targetAsset: toFlashApiAssetAddress(targetAsset),
    contraAsset: toFlashApiAssetAddress(contraAsset),
    from: {
  ...
  quote.raw = response
  quotes.set(quoteId || bridgeQuoteId, { body, quote, response })
...
function orderResponse(order: LocalOrderRecord) {
  ...
  const triggers: unknown[] = Array.isArray(localParameters.triggers) ? localParameters.triggers : []
  ...
  const assetRef = (asset: FlashAsset) => ({
    ...
    ticker: asset.symbol,
    chain: {
      id: getFlashChainSlug(asset.chainId),
      name:
        getFlashChainSlug(asset.chainId) === LOCAL_CHAIN_SLUG ? 'Anvil' : getFlashChainSlug(asset.chainId),
      namespace: 'eip155'
    }
  })
...
function validateSubmitBody(quoteRecord: LocalQuoteRecord, body: Record<string, unknown>) {
  ...
  const expectedTargetAsset =
    wrappedAsset && quoteBody.side === 'sell' ? wrappedAsset : stringValue(quoteResponse.targetAsset)
  const expectedContraAsset =
    wrappedAsset && quoteBody.side === 'buy' ? wrappedAsset : stringValue(quoteResponse.contraAsset)
  const expectedFields: Record<string, unknown> = {
    ...
    recipientAddress: quoteBody.recipientAddress,
  ...
  const expectedQuoteId = stringValue(quoteResponse.quoteId)
  const expectedBridgeQuoteId = stringValue(quoteResponse.bridgeQuoteId)
  if (stringValue(body.quoteId) !== expectedQuoteId) {
    validationError('Local Flash submit quoteId must match the quote response')
  }
  if (stringValue(body.bridgeQuoteId) !== expectedBridgeQuoteId) {
    validationError('Local Flash submit bridgeQuoteId must match the quote response')
  }

  for (const [field, expected] of Object.entries(expectedFields)) {
    if (expected === undefined && body[field] === undefined) {
      continue
    }
...
function storeOrder(quoteRecord: LocalQuoteRecord, body: Record<string, unknown>) {
  ...
  const order: LocalOrderRecord = {
    accountAddress: checksumAddress(
      stringValue(quoteRecord.body.funderAddress) || stringValue(body.funderAddress),
      'funder'
    ),
    bridgeQuoteId: stringValue(quoteRecord.response.bridgeQuoteId) || undefined,
  ...
  if (orderType === FLASH_MARKET_ORDER_TYPE && !chains.isCrossChain) {
    setTimeout(() => {
...
async function fillMarketOrder(orderId: string) {
  ...
  if (order?.status !== 'accepted' || order.orderType !== FLASH_MARKET_ORDER_TYPE) {
    return
  }
  ...
  try {
    ...
    if (receipt?.status !== 1) {
      throw new Error(`Local Flash settlement failed: ${tx.hash}`)
    }
...
function filterOrders(url: URL) {
  const funder = String(url.searchParams.get('funderAddress') ?? '')
    ...
    .toLowerCase()
  if (!funder) {
    validationError('Local Flash order list requires funderAddress')
  }
  ...
  return Array.from(orders.values())
    .filter((order) => {
      if (funder && order.accountAddress.toLowerCase() !== funder) {
        return false
      }
      if (statuses.size && !statuses.has(order.status)) {
        return false
      }
      return true
    })
...
export async function handleLocalTradeRequest(req: Request) {
  try {
    ...
    if (orderRoute && req.method === 'GET' && !orderRoute.action) {
      ...
      if (!funderAddress) {
        return errorResponse('Local Flash order lookup requires funderAddress', 400)
      }
    ...
    if (orderRoute && req.method === 'POST' && orderRoute.action === 'cancel') {
      ...
      if (!order.open || !order.cancellable) {
        return errorResponse('Local Flash order is not cancellable', 409)
      }
