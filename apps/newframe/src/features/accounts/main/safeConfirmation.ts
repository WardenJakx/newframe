...
export function createSafeConfirmationService({
  ...
  const usable = (entry: Entry) => {
    if (disposed || entry.controller.signal.aborted || !entry.context.isOwnerActive()) {
      return false
    }
    const main = store.getState().main
    if (main.appLock.locked || main.currentAccount !== entry.identity.accountId) {
      return false
    }
  ...
  const finish = (entry: Entry, status: Terminal, message?: string) => {
    ...
    for (const reference of entry.references) {
      if (status === 'published') {
        operations.complete(reference, status)
      } else {
        operations.fail(
          reference,
  ...
  const advance = (entry: Entry, phase: 'signing' | 'publishing' | 'reconciling') => {
    ...
    entry.phase = phase
    for (const reference of entry.references) {
      operations.advance(reference, { phase })
    }
  }
            ...
            account.signTypedData(
              ...
              (error, signature) => {
                if (error) {
                  ...
                  reject(new Error('Wallet returned no confirmation.'))
                } else {
                  resolve(signature)
                }
              },
  ...
  const unsubscribe = store.subscribe(
    ...
    () => {
      for (const entry of entries.values()) {
        if (entry.snapshot && !semanticValid(entry)) {
          cancel(entry, true)
        } else if (entry.running && !usable(entry)) {
          cancel(entry)
        }
      }
    }
  ...
  return {
    confirm(command: SafeApprovalCommand, context: SigningUiContext) {
      ...
      if (operations.lookup(reference)) {
        return accepted.get(requestKey) === key
      }
      if (
        previous &&
      ...
      if (!addReference(entry, reference)) {
        return false
      }
      accepted.set(requestKey, key)
      previous?.cleanup()
    ...
    confirmationStatus(query: SafeConfirmationStatusQuery, owner?: OperationOwner): SafeConfirmationStatus {
      ...
      if (!entry || entry.anchor !== anchor(identity)) {
        return { status: 'idle' }
      }
