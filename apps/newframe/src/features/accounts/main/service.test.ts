...
describe('account mutation service', () => {
  it('revalidates account, signer, order, selection, and permission preconditions', async () => {
    ...
    expect(service.update({ type: 'account.update', accountId: 'missing', name: 'Name' })).toBeFalse()
    expect(service.update({ type: 'account.update', accountId: first, name: 'Name' })).toBeTrue()
