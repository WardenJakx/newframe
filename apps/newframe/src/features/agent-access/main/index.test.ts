...
import type { AgentAccessRequest } from '../../requests/contract/requests'
...
it('characterizes agent prompt timeout, disconnect, approval idempotency, and dispose cleanup', async () => {
  ...
  try {
    const requests: Record<string, AgentAccessRequest> = {}
    ...
    const account = {
      ...
      patch: mock(),
      rejectRequest(request: AgentAccessRequest, error: EVMError) {
      ...
      resolveRequest(request: AgentAccessRequest, result: unknown) {
        requestLifecycle.respond(request.handlerId, {
          id: request.payload.id,
          jsonrpc: request.payload.jsonrpc,
          result
    ...
    const accounts = {
      ...
      routeRequest: (_principal: unknown, routed: AgentAccessRequest) => {
