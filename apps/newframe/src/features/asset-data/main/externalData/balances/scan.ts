...
function createBalance(rawBalance: string, decimals: number): ExternalBalance {
  return {
    balance: rawBalance,
    displayBalance: formatUnits(BigInt(rawBalance), decimals)
  }
}

export default function (eth: Eip1193Provider) {
  function balanceCalls(owner: string, tokens: TokenDefinition[]): Call<bigint, ExternalBalance>[] {
    return tokens.map((token) => ({
      ...
      returns: [
        (bn?: bigint) => {
          const hexString = bn ? toBeHex(bn) : '0x00'
