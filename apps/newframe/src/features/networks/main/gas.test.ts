...
import type { Block } from './gas'

// real function under test, exercised with partial fee history fixtures
type TestBlock = Pick<Block, 'baseFee' | 'rewards'> & Partial<Pick<Block, 'gasUsedRatio'>>
const createGasCalculator = (chainId?: string) => {
  const calculator = createGasCalculatorTyped(chainId as string)
  return {
    calculateGas: (blocks: TestBlock[]) => calculator.calculateGas(blocks as Block[])
  }
}
...
describe('#createGasCalculator', () => {
  describe('default gas calculator', () => {
    ...
    it('calculates the base fee for the next block', () => {
      ...
      const { maxBaseFeePerGas } = gasCalculator.calculateGas(feeHistory)
    ...
    it('calculates the priority fee for the next block based on normal blocks', () => {
      ...
      const feeHistory = [
        ...
        {
          baseFee: 8,
          gasUsedRatio: 0.23114498292513627,
          rewards: [2000000000]
        },
        {
          baseFee: 8,
          gasUsedRatio: 0.17942918604838942,
          rewards: [1000000000]
        },
        {
          baseFee: 8,
          gasUsedRatio: 0.12024061496050893,
          rewards: [4000000000]
        },
        { baseFee: 182, rewards: [] }
      ]

      const { maxPriorityFeePerGas } = gasCalculator.calculateGas(feeHistory)
    ...
    it('excludes "empty" blocks from the priority fee calculation', () => {
      ...
      const feeHistory = [
        ...
        {
          baseFee: 8,
          gasUsedRatio: 0.01024061496050893,
          rewards: [4000000000]
        },
        { baseFee: 182, rewards: [] }
      ]
    ...
    it('considers full blocks if no partial blocks are eligible', () => {
      ...
      const feeHistory = [
        ...
        {
          baseFee: 8,
          gasUsedRatio: 0.03114498292513627,
          rewards: [1000000000]
        },
        {
          baseFee: 8,
          gasUsedRatio: 0.07942918604838942,
          rewards: [1000000001]
        },
    ...
    it('uses the priority fee from the latest block when no eligible blocks are available', () => {
      ...
      const { maxBaseFeePerGas, maxPriorityFeePerGas } = gasCalculator.calculateGas(feeHistory)
      expect(maxBaseFeePerGas).toBe('0xe7')
  ...
  describe('polygon gas calculator', () => {
    ...
    it('should enforce a minimum of 30 gwei for the priority fee', () => {
      const { maxPriorityFeePerGas } = gasCalculator.calculateGas([
        {
          baseFee: 8,
          gasUsedRatio: 0.07942918604838942,
          rewards: [1000000000]
    ...
    it('does not change the priority fee if above 30 gwei', () => {
      const gasCalculator = createGasCalculator('137')
