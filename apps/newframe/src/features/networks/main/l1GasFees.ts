...
export async function estimateL1GasCost(provider: RpcProvider, txData: L1TxParams): Promise<bigint> {
  ...
  const result = await provider.send('eth_call', [{ to: GAS_PRICE_ORACLE_ADDRESS, data: calldata }, 'latest'])
