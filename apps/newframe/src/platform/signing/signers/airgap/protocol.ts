...
function inspectCbor(bytes: Buffer): Value {
  if (!bytes.length || bytes.length > MAX_MESSAGE) throw invalid()
  let cursor = 0
  let items = 0
  const walk = (depth: number): Value => {
    if (depth > 16 || ++items > 4096 || cursor >= bytes.length) throw invalid()
    ...
    if (info >= 28) throw invalid()
    let size = info
    if (info >= 24) {
      const width = 2 ** (info - 24)
      if (width > bytes.length - cursor) throw invalid()
      let value = 0n
      for (let i = 0; i < width; i++) value = (value << 8n) | BigInt(bytes[cursor++])
      if (value > BigInt(Number.MAX_SAFE_INTEGER)) throw invalid()
      size = Number(value)
    }
    if (major === 0) return size
    if (major === 1) {
      if (!Number.isSafeInteger(-1 - size)) throw invalid()
      return -1 - size
    }
    if (major === 2 || major === 3) {
      if (size > bytes.length - cursor) throw invalid()
      const value = bytes.subarray(cursor, cursor + size)
      cursor += size
      return major === 2 ? value : new TextDecoder('utf-8', { fatal: true }).decode(value)
    }
    if (major === 4) {
      if (size > 4096 - items || size > bytes.length - cursor) throw invalid()
      const values: Value[] = []
      for (let i = 0; i < size; i++) values.push(walk(depth + 1))
      return values
    }
    if (major === 5) {
    ...
    if (major === 6 && [37, 304, 305].includes(size)) return { tag: size, value: walk(depth + 1) }
    throw invalid()
  }
  const result = walk(0)
...
function map(value: Value | undefined, keys: number[]): Map<number, Value> {
  if (!(value instanceof Map) || [...value.keys()].some((key) => !keys.includes(key))) throw invalid()
  return value
}
...
function byteString(value: Value | undefined, min: number, max = min): Buffer {
  if (!Buffer.isBuffer(value) || value.length < min || value.length > max) throw invalid()
  return value
}
function keypath(value: Value | undefined, origin: boolean) {
  const path = map(tagged(value, 304), [1, 2, 3])
  const components = path.get(1)
  if (!Array.isArray(components) || components.length !== (origin ? 6 : 4)) throw invalid()
  for (let i = 0; i < components.length; i += 2) {
    uint(components[i], 0x7fffffff)
    if (components[i + 1] !== origin) throw invalid()
  }
  if (origin && (components[0] !== 44 || components[2] !== 60)) throw invalid()
  if (!origin && (components[0] !== 0 || components[2] !== 0)) throw invalid()
  if (origin || path.has(2)) uint(path.get(2))
  if (path.has(3) && uint(path.get(3)) !== (origin ? 3 : 2)) throw invalid()
}
function inspectProfile(cbor: Buffer, type: 'crypto-hdkey' | 'eth-signature') {
  ...
  if (type === 'eth-signature') {
    ...
    if (record.has(3) && typeof record.get(3) !== 'string') throw invalid()
    return record
  }
  for (const field of [1, 2]) if (record.has(field) && record.get(field) !== false) throw invalid()
  byteString(record.get(3), 33)
  byteString(record.get(4), 32)
  keypath(record.get(6), true)
  if (record.has(7)) keypath(record.get(7), false)
  if (record.has(5)) {
    const info = map(tagged(record.get(5), 305), [1, 2])
    if (uint(info.get(1) ?? 0) !== 60 || uint(info.get(2) ?? 0) !== 0) throw invalid()
  }
  if (record.has(8)) uint(record.get(8))
  for (const key of [9, 10]) if (record.has(key) && typeof record.get(key) !== 'string') throw invalid()
  return record
}
...
export class AirGapUrAssembler {
  ...
  receive(frame: string): Buffer | undefined {
    if (typeof frame !== 'string' || frame.length > 4096 || !/^[\x21-\x7e]+$/.test(frame)) throw invalid()
    const [type, components] = URDecoder.parse(frame)
    if (type !== this.type) throw new Error(`Scan a ${this.type} QR.`)
    if (components.length !== 1 && components.length !== 2) throw invalid()
    const canonical = frame.toLowerCase()
    const body = components.at(-1)!
    if (!/^(?:[a-z]{2})+$/.test(body)) throw invalid()
    ...
    if (components.length === 2) {
      if (!/^[1-9]\d{0,9}-[1-9]\d{0,2}$/.test(components[0])) throw invalid()
      const [outerSequence, outerCount] = components[0].split('-').map(Number)
      if (outerSequence > UINT32 || outerCount > 512) throw invalid()
      const part = inspectCbor(cbor)
      if (!Array.isArray(part) || part.length !== 5) throw invalid()
    ...
    } else inspectProfile(cbor, this.type)
    if (this.frames.has(canonical)) return
...
export function decodePublicAccount(cbor: Buffer): AirGapPublicAccount {
  ...
  if (record.isMaster() || record.isPrivateKey()) throw invalid()
...
export function deriveAirGapAddresses(record: AirGapPublicAccount): string[] {
  ...
  if (!branch.publicKey || !branch.chainCode) throw invalid()
...
export function chainNumber(value: string | number | bigint) {
  ...
  if (chain < 1n || chain > BigInt(Number.MAX_SAFE_INTEGER)) throw new Error('Unsupported AirGap chain ID')
  return Number(chain)
}
export function transactionPreimage(tx: TypedTransaction) {
  if (tx.type !== 0 && tx.type !== 2) throw new Error('AirGap supports legacy and type 2 transactions only')
  if (!tx.to) throw new Error('AirGap does not support contract creation')
...
export function requestFrames(
  ...
  if (signData.length > MAX_MESSAGE) throw new Error('AirGap request is too large')
  const components = [
    ...record.originPath
      .slice(2)
      .split('/')
  ...
  if (ur.cbor.length > MAX_MESSAGE) throw new Error('AirGap request is too large')
  ...
  if (count * fragmentLength > MAX_MESSAGE) throw new Error('AirGap request is too large')
...
export function decodeSignature(cbor: Buffer, sessionId: string, kind: DataType, chainId: number) {
  ...
  if (kind === DataType.personalMessage && (rawV === 0n || rawV === 1n)) rawV += 27n
  const base =
