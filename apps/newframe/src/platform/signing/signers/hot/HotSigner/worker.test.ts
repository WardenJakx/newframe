...
import HotSignerWorker from './worker'
...
describe('#signTransaction', () => {
  ...
  chains.forEach((chain) => {
    it(`signs a transaction on ${chain.name}`, (done) => {
      ...
      worker.signTransaction(key, tx, (err: any, signature: any) => {
        try {
  ...
  it('rejects a transaction with an unknown chain id', (done) => {
    const { chainId, ...tx } = rawTx as any

    worker.signTransaction(key, tx, (err: any) => {
      try {
