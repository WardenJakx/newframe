...
// keep this in sync with src/features/networks/main/config — duplicated here because the worker
...
class HotSignerWorker {
  ...
  constructor() {
    ...
    setImmediate(() => this.sendToken())
  }

  sendToken() {
    this.send({ type: 'token', token: this.token })
  }

  send(message: unknown) {
    try {
      if (process.connected && (process as any).channel) process.send!(message)
    } catch (e) {
      // Parent may have closed the worker while a response was in flight.
    }
  }

  handleMessage(message: WorkerRPCMessage | { type: string }) {
    if ('type' in message && message.type === 'getToken') {
      this.sendToken()
      return
    }

    const { id, method, params, token } = message as WorkerRPCMessage
  ...
  verifyAddress({ index, address }: { index: number; address: string }, pseudoCallback: PseudoCallback) {
    ...
    ;(this.signMessage as any)({ index, message }, (err: unknown, signedMessage: string) => {
      ...
      if (Buffer.from(signedMessage.replace('0x', ''), 'hex').length !== 65)
        return pseudoCallback(new Error('Newframe verifyAddress signature has incorrect length'))
      // Verify address
