...
type ConnectedHid = { interface: number; path: string; product: string; usagePage: number }
...
let LedgerSignerAdapter: typeof import('./adapter').default
type AdapterTestView = Pick<EventEmitter, 'off' | 'on' | 'once'> & {
  close(): void
...
beforeEach(() => {
  ...
  adapter = new LedgerSignerAdapter(store) as unknown as AdapterTestView
  adapter.open()
})
...
function nextEvent<T = LedgerMock>(event: string, predicate: (value: T) => boolean = () => true) {
...
it('handles a disconnected Ledger', async () => {
  const connected = nextEvent('update', (ledger) => ledger.status === Status.OK)
