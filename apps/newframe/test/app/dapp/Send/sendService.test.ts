...
import { closeSend, resolveName, submitTransaction } from '../../../../app/dapp/Send/sendService'
...
describe('Send commands', () => {
  ...
  it('closes only its own dapp window', () => {
    closeSend()
    expect(link.executeCommand).toHaveBeenCalledWith({ type: 'dapp.close' })
  })
})
