...
beforeAll(async () => {
  ...
  init = (await import('../../../main/errors')).init
...
beforeEach(() => {
  store.set('main.networks', {})
  store.set('main.networksMeta', {})
...
describe('sentry', () => {
  describe('#beforeSend', () => {
    const sendErrorEvent = (event: any) => SentryInstance.beforeSend(event)
    const sendErrorEvents = (numEvents: any) => [...Array(numEvents).fill(validEvent)].map(sendErrorEvent)
    const simulateException = (ex: any) => sendErrorEvent({ exception: { values: [ex] } })

    let SentryInstance: any

    beforeEach(() => {
      init()
      SentryInstance = (Sentry.init as any).mock.calls[0][0]
    })

    const sampleException = {
      type: 'Error',
      value:
        'Cannot find latest.yml in the latest release artifacts (https://github.com/wardenjakx/newframe/releases/download/v0.5.0-beta.20/latest.yml): HttpError: 404 \n"method: GET url: https://github.com/wardenjakx/newframe/releases/download/v0.5.0-beta.20/latest.yml\\n\\...',
      stacktrace: {
        frames: [
          {
            module:
              'C:\\Users\\RickyBobby\\AppData\\Local\\Programs\\newframe\\resources\\app.asar\\node_modules\\electron-updater\\out\\AppUpdater'
          },
          { module: 'node:domain' },
          {
            module:
              'C:\\Users\\RickyBobby\\AppData\\Local\\Programs\\newframe\\resources\\app.asar\\compiled\\main\\signers\\lattice\\Lattice\\index'
          }
        ]
      }
    }
    ...
    it('should strip asar paths from stackframe modules on Windows', () => {
      ...
      const stackFrameModules = sentryEvent.exception.values[0].stacktrace.frames.map(
        (frame: any) => frame.module
      )
      ...
      const linuxException = {
        ...
        stacktrace: {
          frames: [
            {
              module:
                '/home/RickyBobby/.config/newframe/resources/app.asar/node_modules/electron-updater/out/AppUpdater'
            },
            { module: 'node:domain' },
            {
              module:
                '/home/RickyBobby/.config/newframe/resources/app.asar/compiled/main/signers/lattice/Lattice/index'
            }
          ]
        }
      }
