...
beforeAll(async () => {
  ...
  Eth = (await import('../../../../../main/signers/ledger/Ledger/eth')).default
})

beforeEach(async () => {
  ;(Eth as any).mockClear()
  ethInstance = undefined

  ledger = new (Ledger as any)('usb-path')
  ledger.derivation = Derivation.legacy

  await ledger.open()
  ethInstance.deriveAddresses.mockImplementation(() => Promise.resolve(addresses))
})
...
describe('#deriveAddress', () => {
  ...
  it('derives live addresses', (done) => {
    ethInstance.getAddress.mockImplementation((path: any) => {
...
describe('#verifyAddress', () => {
  beforeEach(async () => {
    await connectEthApp()
    ethInstance.getAddress.mockImplementation(
      (path: any) =>
        new Promise((resolve) => {
          resolve({
            address:
  ...
  const errorCases = [
    ...
    },
    {
      ...
      setup: () => ethInstance.getAddress.mockRejectedValue({ statusCode: 27013 })
    },
    {
      testCase: 'there is a communication error',
      setup: () => ethInstance.getAddress.mockRejectedValue({ statusCode: -1 })
    },
    {
...
signingMethods.forEach((signingMethod) => {
  ...
  describe(`#${signingMethod}`, () => {
    ...
    it(`signs a ${signType}`, (done) => {
      ethInstance[signingMethod].mockImplementation(
        (path: any) =>
          new Promise((resolve) => {
            resolve(
              path === "44'/60'/0'/3"
    ...
    const errorCases = [
      {
        testCase: 'there is a communication error',
        setup: () => ethInstance[signingMethod].mockRejectedValue({ statusCode: -1 })
      },
      {
        testCase: 'the eth app is not initialized',
        setup: () => (ledger.eth = undefined)
...
describe('#signTypedData', () => {
  ...
  it('signs v4 typed data', (done) => {
    ethInstance.signTypedData.mockImplementation(
      (path: any) =>
        new Promise((resolve) => {
          resolve(
            path === "44'/60'/0'/5"
  ...
  it('fails if the signing request is invalid', (done) => {
    ethInstance.signTypedData.mockRejectedValue({
      statusCode: 99901,
  ...
  const errorCases = [
    {
      testCase: 'there is a communication error',
      setup: () => ethInstance.signTypedData.mockRejectedValue({ statusCode: -1 })
    },
    {
      testCase: 'the eth app is not initialized',
      setup: () => (ledger.eth = undefined)
