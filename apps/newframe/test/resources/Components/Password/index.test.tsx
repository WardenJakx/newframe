...
describe('creating password', () => {
  const setupComponent = ({ onCreate = jest.fn() } = {}) => {
    ...
    return {
      ...
      enterPassword: async (text: string) => user.type(screen.getByRole('textbox'), text)
    }
  }
  ...
  it('should show the warning when the password is too weak', async () => {
    ...
    expect(getSubmitButton().textContent).toBe('REPEATED CHARACTERS LIKE "AAA" ARE EASY TO GUESS.')
  })
