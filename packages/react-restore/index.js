...
const freezeDeep = (o) => {
  if (typeof o === "object" && o !== null)
    Object.keys(o).forEach((k) => freezeDeep(o[k]));
...
const expand = (internal) => {
  ...
  const peel = (point) => {
    if (point) {
      if (internal.links[point] && targets.indexOf(point) === -1)
        targets.push(point);
  ...
  paths.forEach((path) => {
    links.forEach((link) => {
      if (link.startsWith(path) && targets.indexOf(link) === -1)
        targets.push(link); // Target child link
    });
...
const observe = (internal, id, run) => {
  ...
  const self = {
    ...
    remove: () => internal.store.api.remove(id),
  };
...
const notify = (internal) => {
  ...
  internal.pending.sort(
    (a, b) => internal.order.indexOf(a) - internal.order.indexOf(b),
  );
  Object.keys(internal.watchers).forEach((id) =>
    internal.watchers[id](
      internal.state,
      internal.queue.actions,
      internal.pending.length,
    ),
  );
...
const patch = (obj, path, value) => {
  ...
  obj[key] =
    path.length > 0 ? patch(obj[key], path, value) : freeze.deep(value);
...
const resolve = (internal, action, tree = {}, name) => {
  if (typeof action === "function") {
    return (...args) => {
      ...
      if (internal.queue.actions.length === 1)
        setTimeout(() => notify(internal), 0);
      const update = (...args) => {
        args = [...args];
        const up = args.pop();
        const path = pathway(args) || "*";
        const value = up(
          thaw.deep(path === "*" ? internal.state : get(internal.state, path)),
          internal.state,
        );
    ...
    throw new Error(
      `[Restore] Invalid entry in action tree: '${name}' is a ${typeof action}.`,
    );
  }
  return tree;
};
...
export const create = (state = {}, actions = {}) => {
  ...
  const store = (...args) => {
    ...
    if (internal.track) {
      ...
      if (internal.observers[id].links.indexOf(path) === -1)
        internal.observers[id].links.push(path);
      if (internal.links[path].indexOf(internal.track) === -1)
        internal.links[path].push(internal.track);
    }
  ...
  store.observer = (run, id, alt) => {
    ...
    return {
      returned: observe(internal, id, run),
      remove: () => store.api.remove(id),
    };
  };
  store.api = {
    replaceState: (state) => {
  ...
  Object.keys(store.api).forEach((method) => {
    if (actions[method])
      throw new Error(`[Restore] API method name ${method} is reserved.`);
  });
...
export const connect = (Component, store) => {
  ...
  class Connected extends Component {
    ...
    render(...args) {
      const observer = this.store.observer(
        super.render.bind(this, ...args),
