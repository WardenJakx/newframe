...
const report: AuditReport = {
  ...
  storeVariantCount: sum(
    [...storePackages.values()].map((pkg) => pkg.variants),
  ),
  ...
  topDirectBySize: args.skipSize
    ? []
    : top(directDependencies, args.top, (dep) => dep.approxBytes),
...
function collectDuplicatePackages(
  ...
  return [...packagesByName.entries()]
    .map(([name, nodes]) => {
      const versions = [...new Set(nodes.map((node) => node.version))]
...
async function readBunStore(storeRoot: string) {
  ...
  for (const entry of entries) {
    ...
    const packageJsonPath = packageJsonPathForStorePackage(
      storeRoot,
      entry.name,
      parsed.name,
    );
...
function parseStoreDirectoryName(directoryName: string) {
  ...
  const version =
    peerHashSeparator === -1
      ? versionAndPeerHash
      : versionAndPeerHash.slice(0, peerHashSeparator);
...
function packageJsonPathForStorePackage(
  storeRoot: string,
...
function resolveDependency(
  ...
  const exact = candidates.find(
    (candidate) =>
      candidate.version === range || candidate.id === `${name}@${range}`,
  );
  if (exact) return exact;

  const satisfying = candidates.filter((candidate) =>
    satisfiesRange(candidate.version, range),
  );
...
function printReport(report: AuditReport, options: typeof args) {
  ...
  console.log("");
  if (!options.skipSize) {
    printDirectDependencyTable(
      "Direct Dependencies By Approx Transitive Store Size",
      report.topDirectBySize,
    );
    console.log("");
  }
  printDirectDependencyTable(
    "Direct Dependencies By Transitive Fanout",
    report.topDirectByFanout,
  );

  if (!options.skipSize) {
    console.log("");
    printStoreTable(
      "Largest Required Store Packages",
      report.topRequiredStorePackages,
    );
    console.log("");
    printStoreTable(
      "Largest Store Packages Not In Current Lockfile",
      report.topStaleStorePackages,
    );
  }
  ...
  console.log(
    "- Stale store packages occupy disk but are not required by the current lockfile.",
  );
  console.log(
    "- Run from a clean install for the most accurate disk-size report.",
  );
}

function printDirectDependencyTable(
  title: string,
  dependencies: DirectDependency[],
) {
  console.log(title);
  console.log(
    "size       deps  workspace             kind                  package",
  );
...
function printStoreTable(title: string, packages: StorePackage[]) {
  ...
  for (const pkg of packages) {
    console.log(
      `${formatSize(pkg.bytes).padStart(9)}  ${String(pkg.variants).padStart(8)}  ${pkg.id}`,
    );
  }
}
